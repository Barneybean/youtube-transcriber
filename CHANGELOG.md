...
- **AI transcript proofreading** — after transcription, Whisper output is proofread by Anthropic Claude (`lib/proofread.ts`): fixes ASR errors (homophones, garbled domain terms) without rephrasing, preserving all segment timing. BYOK via `ANTHROPIC_API_KEY`; configure with `PROOFREAD_ENABLED` / `PROOFREAD_MODEL` (default `claude-opus-4-8`) / `PROOFREAD_SOURCES` (`whisper` default, `all` to include caption tracks). Fails open — transcription is never blocked by proofreading errors. Runs for YouTube, Spotify, and generic-video pipelines.

### Changed

- **Local Whisper default model is now `large-v3-turbo`** (was `base`) — near `large-v3` accuracy at ~8× its speed; `base` garbled non-English finance/tech speech. Override with `WHISPER_MODEL` in `.env` (e.g. `WHISPER_MODEL="base"` for quick low-accuracy runs). First run downloads the new model (~1.6 GB).

## 2026-07-17

### Added

- **Groq-first audio fallback** — YouTube captions remain the fixed first transcription path; when captions are unavailable, configured Groq runs first, OpenAI's hosted `whisper-1` runs next, other cloud providers follow their configured order, and local Whisper runs last.
- **Per-file transcriber audit trail** — Exported Markdown, library downloads, and batch JSON/TXT summaries now identify the transcription source used for each file.
- **Existing-export metadata backfill** — Added `npm run backfill-transcriber-sources` and labeled all 81 existing transcript files from their recorded database source.

### Changed

- **Provider settings** — OpenAI can now be configured directly with an API key. Groq and OpenAI are visibly pinned after YouTube captions in that order, and the UI clarifies that OpenAI API billing is separate from ChatGPT subscriptions.
- **Supported Node runtime documented** — Added `.nvmrc` and package engine metadata for Node 24 because the current SQLite native dependency is not compatible with Node 26.

## 2026-07-16

### Added

- **File-first YouTube export workflow** — Paste a YouTube video or channel URL to discover videos, transcribe them sequentially, and save timestamped Markdown under `Desktop/Youtube_Transcript/<Channel Name>/`. Each channel folder includes an inventory plus JSON and text summaries.
- **Safe batch resume** — Channel exports reuse stored transcripts, skip files already present in the destination, retry transient failures, checkpoint after every video, and can stop safely after the current item.
- **Members-only channel support** — Channel discovery and restricted audio downloads use the signed-in Chrome profile, with automatic Chrome-session refresh attempts when YouTube rotates authentication.

### Changed

- **Focused Transcript Desk interface** — Replaced the library-first home screen with one primary workflow: URL input, explicit local destination, progress, recent outcomes, stop control, and Finder access. Library, summarization, provider, and promotional controls no longer compete with transcription on the primary surface.

### Fixed

- **Members-only audio authentication retry** — Local Whisper now recognizes YouTube membership and rotated-cookie errors as authentication failures and retries with the configured Chrome profile.
